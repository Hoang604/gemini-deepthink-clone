import { Message, ModelConfig, GeminiModel, Strategy, ThinkingProcess, ExecutionStep, MasterBlueprint } from '../types';
...
export const orchestrateDeepThink = async (
  ...
  try {
    ...
    addTraceStep({ id: divStepId, phase: 'Divergence', title: 'Path Generation', status: 'running' });
    onUsage(usageIncrement);

    const context = history.length > 0 ? getCondensedHistory(history) : undefined;
    const divergenceResponse = await generateContent(config.model, Prompts.GET_DIVERGENCE_PROMPT(currentMessage, context), true);
    ...
    try { 
      ...
      strategies = [{ id: "default", title: "Standard Analysis", strategy: "Direct execution.", assumption: "Direct response" }];
    }
    
    updateTraceStep(divStepId, { status: 'complete', thoughts: `Explored ${strategies.length} strategic paths.`, result: strategies });
    ...
    const critiqueSingle = async (s: Strategy, stepId: string): Promise<Strategy> => {
      ...
      try { critiqueResult = JSON.parse(critiqueText || "{}"); } catch(e) {}
      
      updateTraceStep(stepId, { status: 'complete', thoughts: critiqueResult.critical_flaws, result: critiqueResult });
      return { ...s, critique: critiqueResult };
    };

    const strategiesWithSteps = strategies.map(s => ({ ...s, stepId: generateId() }));
    strategiesWithSteps.forEach(s => addTraceStep({ id: s.stepId, phase: 'Critique', title: `Testing: ${s.title}`, status: 'running' }));

    const critiquedStrategies = await Promise.all(strategiesWithSteps.map(s => critiqueSingle(s, s.stepId)));
    ...
    addTraceStep({ id: synthStepId, phase: 'Synthesis', title: 'Blueprint Synthesis', status: 'running' });
    onUsage(usageIncrement);

    const synthesisResponse = await generateContent(config.model, Prompts.GET_MASTER_SYNTHESIS_PROMPT(currentMessage, critiquedStrategies), true);
    ...
    updateTraceStep(synthStepId, { status: 'complete', thoughts: `Master Blueprint synthesized with ${masterBlueprint.safeguards.length} safeguards.`, result: masterBlueprint });
    updateState({ masterBlueprint });

    return { 
      prompt: Prompts.GET_FINAL_GENERATION_PROMPT(currentMessage, masterBlueprint),
...
export const streamGeminiResponse = async (
  ...
  const mappedHistory = history.map(m => {
    ...
    if (m.role === 'model' && m.thinkingProcess && m.thinkingProcess.state === 'complete') {
      textContent = `${summarizeThinking(m.thinkingProcess)}\n\n${m.text}`;
    }
    return { role: m.role, parts: [{ text: textContent }] };
  });

  const chat = createChatSession(config.model, mappedHistory, config.temperature);
