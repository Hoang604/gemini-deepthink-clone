...
interface WorkbenchProps {
  ...
  onUpdateType?: (artifactId: string, type: Artifact["type"]) => void;
  ...
  const getLanguage = (type: string) => {
    switch (type) {
      ...
      case "html":
        return "markup";
      case "mermaid":
        return "clike"; // Prism doesn't have mermaid by default, clike is a safe fallback
      case "c":
        return "c";
      default:
        return "clike";
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#1e1f20] text-gray-500 rounded-2xl border border-white/5 p-8 shadow-2xl">
        ...
        <p className="text-sm text-gray-500 text-center max-w-xs mb-8">
          The model hasn't generated any code yet, or you haven't started a
          scratchpad.
        </p>
    ...
    <div className="h-full flex flex-col bg-[#1e1f20] rounded-2xl border border-white/5 shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-[#1e1f20] border-b border-white/5 flex-none h-16">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="flex flex-col">
            ...
            <select
              value={artifact.type}
              onChange={(e) =>
                onUpdateType?.(artifact.id, e.target.value as any)
              }
              className="text-[10px] text-gray-500 font-mono bg-transparent border-none outline-none cursor-pointer hover:text-[#a8c7fa] transition-colors appearance-none"
            >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleCopy}
            className="p-2 hover:bg-[#282a2c] rounded-full text-gray-400 hover:text-white transition-colors"
          >
            {copied ? (
            ...
            <div className="flex-1 relative overflow-auto custom-scrollbar flex bg-[#131314]">
              ...
              <div className="flex-none w-12 bg-[#131314] border-r border-white/5 text-right py-4 pr-3 select-none">
                {lineNumbers.map((n) => (
                  <div
                    key={n}
                    className="text-[14px] leading-[1.6] text-gray-500 font-mono font-medium"
                  >
                    {n}
                  </div>
                ))}
